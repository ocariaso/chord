...
import {
  CABINET_BEZEL_PADDING,
  CABINET_BEZEL_PADDING_MOBILE,
  CABINET_GAP,
  CABINET_GAP_MOBILE,
  CABINET_INTERIOR_COLOR,
  CABINET_INTERIOR_IMAGE,
  CABINET_PADDING_X_MOBILE,
  CABINET_POST_WIDTH,
  CABINET_POST_WIDTH_MOBILE,
  CABINET_WIDTH,
} from "./constants";
...
export function StudioCabinet({ children, isMobile = false }: { children: ReactNode; isMobile?: boolean }) {
  const gap = isMobile ? CABINET_GAP_MOBILE : CABINET_GAP;
  const postWidth = isMobile ? CABINET_POST_WIDTH_MOBILE : CABINET_POST_WIDTH;
  const bezelPadding = isMobile ? CABINET_BEZEL_PADDING_MOBILE : CABINET_BEZEL_PADDING;
  const paddingX = isMobile ? CABINET_PADDING_X_MOBILE : 22;
  ...
  return (
    <div
      ...
      style={{
        width: "100%",
        maxWidth: CABINET_WIDTH,
        background: WALNUT,
        padding: `26px ${paddingX}px`,
        boxShadow: "0 30px 70px rgba(0,0,0,0.75), inset 0 2px 4px rgba(255,255,255,0.05)",
      }}
    >
      {/* top brass rail with branding plate */}
      <div
        className="relative flex items-center justify-center rounded-[3px]"
        style={{ height: 18, background: BRASS, marginBottom: gap, boxShadow: "inset 0 1px 2px rgba(255,255,255,0.3), inset 0 -1px 2px rgba(0,0,0,0.5)" }}
      >
      ...
      <div className="flex" style={{ gap }}>
        <div
          ...
          style={{ width: postWidth, background: LEATHER, boxShadow: "inset 0 2px 4px rgba(255,255,255,0.06), inset 0 -2px 4px rgba(0,0,0,0.5)" }}
        >
          <BrassRivet />
          <BrassRivet />
        </div>
      ...
      <div
        ...
        style={{ height: 18, background: BRASS, marginTop: gap, boxShadow: "inset 0 1px 2px rgba(255,255,255,0.3), inset 0 -1px 2px rgba(0,0,0,0.5)" }}
      >
        <div className="absolute left-3.5 top-1/2 h-1.5 w-1.5 -translate-y-1/2 rounded-full" style={{ backgroundColor: "#2a1c0e", boxShadow: "inset 0 1px 1px rgba(0,0,0,0.6)" }} />
        <div className="absolute right-3.5 top-1/2 h-1.5 w-1.5 -translate-y-1/2 rounded-full" style={{ backgroundColor: "#2a1c0e", boxShadow: "inset 0 1px 1px rgba(0,0,0,0.6)" }} />
      </div>
