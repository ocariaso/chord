...
| Task | Files |
| --- | --- |
| Add a pipeline stage | [`pipeline/pipeline.py`](../../server/app/pipeline/pipeline.py) — its place in `run_job`, a `stage` name with an entry in `_STAGE_FAILURE_MESSAGES`, a progress step, and an `_is_superseded` checkpoint after it — plus a new leaf module in [`pipeline/`](../../server/app/pipeline/). Follow the leaf contract: no database access. Work that reads only the original can run beside separation: submit it in `_start_analysis`, collect its future in stage order, and write its stage message only if it isn't done — it can't be interrupted by a cancel. For the screen: its label in `processingCopy.stages` ([`design/copy.ts`](../../web/src/design/copy.ts)), which is `PROCESSING_STAGES`, and its index in `processingStage` ([`design/stages.ts`](../../web/src/design/stages.ts)) |
| Change stage progress or messages | [`pipeline/pipeline.py`](../../server/app/pipeline/pipeline.py) — `_SEPARATION_PROGRESS`, `_TEMPO_PROGRESS` and `_CHORDS_PROGRESS` are there. Tempo and chord detection run beside separation, so their steps are written only when they are still running once the stems are — a fast one is never a current stage. A stage message is also a label in `processingCopy.stages` ([`design/copy.ts`](../../web/src/design/copy.ts)), and `processingStage` recognises *Detecting tempo* only by it; rename both together |
| Find where a job's time goes | `docker compose logs -f server` — `_timed` in [`pipeline/pipeline.py`](../../server/app/pipeline/pipeline.py) logs each step's seconds at `INFO` (download, separation, analysis decode, tempo, chords and key, model warm-up) and the job's total; [`main.py`](../../server/app/main.py) sets the level that lets them through |
| Trade separation quality for speed | `DEMUCS_OVERLAP` in the server's environment — `demucs_overlap` in [`core/config.py`](../../server/app/core/config.py), default `0.25`, Demucs' own. A lower value such as `0.1` is faster, with more audible seams where chunks meet. Changing the default means documenting it in [../operations/configuration.md](../operations/configuration.md) |
...
| Change the stored stem format | `STEM_SUFFIX` and the `soundfile.write` call in [`pipeline/separation.py`](../../server/app/pipeline/separation.py); the literal `.flac` in `get_stem`'s route and its `media_type` in [`routes_stems.py`](../../server/app/api/routes_stems.py) (`_wav_stream` decodes whatever libsndfile reads, and assumes 16-bit); `stemUrl` in [`client.ts`](../../web/src/api/client.ts). Existing job directories keep the old format, which `_stems_complete` won't recognise, so their resumes separate again |
| Change the export format | `_wav_stream`, `_wav_size` and `download_stem_wav` in [`routes_stems.py`](../../server/app/api/routes_stems.py), and the entry names in `_zip_stream`; `stemDownloadUrl` in [`client.ts`](../../web/src/api/client.ts); the saved name in [`results/ExportDialog.tsx`](../../web/src/screens/results/ExportDialog.tsx) and `dialogCopy.export` (`hint`, `format`) in [`design/copy.ts`](../../web/src/design/copy.ts) |
| Change how the zip is built | `_zip_stream` and `download_stems` in [`routes_stems.py`](../../server/app/api/routes_stems.py) — keep it streamed through `_ChunkSink`, never buffered, and a plain `def`. Deflate is at `compresslevel=1`: near-silent stems compress well even there |
...
| A cancelled or superseded run's tempo or chord step keeps running on its analysis thread, overlapping the next job | [../architecture/job-lifecycle.md](../architecture/job-lifecycle.md#cancelling) |
