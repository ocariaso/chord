...
| Page | Covers |
| --- | --- |
| [jobs.md](jobs.md) | create, list, read, cancel, discard, and the SSE progress stream |
| [artifacts.md](artifacts.md) | thumbnail, individual stems, the all-stems zip |
| [analysis.md](analysis.md) | chords and lyrics |
...
- **Errors** are FastAPI's default shape, `{"detail": "message"}`. The client prefers that
  message over a generic string when throwing, so `detail` text reaches the user verbatim.
- **202 Accepted** on both create endpoints — work is queued, not done.
...
- **Timestamps** are ISO-8601 UTC strings from `datetime.now(timezone.utc).isoformat()`, stored
  and returned as `TEXT`. The client never parses them.
