...
The last row is easy to miss because it is strings rather than types. The
...
under *Separating stems*, and its headline stops matching its stage list.
...
`templateStems` without a word: never fetched, never shown. Copy that counts or lists the stems — the landing
intro's *six isolated stems* and *"vocals · drums · bass · guitar · piano · other"* — is fixed in
`copy.ts` too. See
...
unreachable server, plus `createJob`'s for a 413. Generated types wouldn't cover the stage messages either.
