...
Expect it to be slow, for two reasons that only apply once:

1. **The image build** compiles madmom from source (`--no-build-isolation`, plus a patch step)
   and installs torch.
2. **The first job** downloads the `htdemucs_6s` weights into `server/data/models_cache/`.

Both are cached afterwards — the weights survive container recreation because that directory is
